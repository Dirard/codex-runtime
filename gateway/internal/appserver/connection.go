...
// ForwardedServerRequestHook may claim a forwarded app-server request before
// it is passed to the legacy task request channel.
type ForwardedServerRequestHook func(ServerRequest) bool
...
type ThreadTurnsListCall struct {
	ThreadID      string
	Cursor        string
	Limit         uint32
...
type Connection struct {
	...
	onForwardedRequest   ForwardedServerRequestHook
}
...
// SetForwardedServerRequestHook updates the optional hook that can claim
// forwarded app-server requests before the task compatibility channel sees
// them. A nil hook disables claiming.
func (c *Connection) SetForwardedServerRequestHook(hook ForwardedServerRequestHook) {
	if c == nil {
		return
	}
	c.hooksMu.Lock()
	defer c.hooksMu.Unlock()
	c.onForwardedRequest = hook
}
...
func (c *Connection) ReadThread(ctx context.Context, call ThreadReadCall) (json.RawMessage, error) {
	return c.dispatcher.Call(ctx, "thread/read", NewThreadReadParams(call.ThreadID, call.IncludeTurns), threadCallTimeout(call.Timeout), CallMetadata{
		TaskID:           call.TaskID,
		SessionGroupID:   c.session.SessionGroupID,
		ExpectedThreadID: call.ThreadID,
...
func (c *Connection) ListThreadTurns(ctx context.Context, call ThreadTurnsListCall) (json.RawMessage, error) {
	return c.dispatcher.Call(ctx, "thread/turns/list", NewThreadTurnsListParams(call.ThreadID, call.Cursor, call.Limit, call.SortDirection, call.ItemsView), threadCallTimeout(call.Timeout), CallMetadata{
		TaskID:           call.TaskID,
		SessionGroupID:   c.session.SessionGroupID,
		ExpectedThreadID: call.ThreadID,
		CloseOnUncertain: false,
...
func (c *Connection) handleServerRequests() {
	for request := range c.dispatcher.Requests() {
		...
		default:
			if c.tryHandleForwardedRequest(request) {
				continue
			}
			if c.requests != nil {
				select {
...
func (c *Connection) forwardedServerRequestHook() ForwardedServerRequestHook {
	c.hooksMu.RLock()
	defer c.hooksMu.RUnlock()
	return c.onForwardedRequest
}

func (c *Connection) tryHandleForwardedRequest(request ServerRequest) bool {
	hook := c.forwardedServerRequestHook()
	if hook == nil {
		return false
	}
	return hook(request)
