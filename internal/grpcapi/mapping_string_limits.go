...
func unknownRawEventMappingFailure(event domain.UnknownRawEvent) *MappingFailure {
	if event.Method == "" {
		return invalidMappingFailure("unknown raw event is missing method")
	}
	return outboundStringMappingFailure(
		outboundStringCheck{field: "unknown raw method", value: event.Method, capBytes: domain.MaxSourceLabelBytes},
		outboundStringCheck{field: "unknown raw payload JSON", value: event.PayloadJSON, capBytes: domain.MaxOutboundUnknownRawJSONBytes},
	)
}
