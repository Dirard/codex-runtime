...
func TestTaskEventToProtoWithFailureRejectsOverCapPublicStrings(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "unknown raw JSON",
			payload: domain.UnknownRawEvent{Method: "unknown/method", PayloadJSON: overCap(domain.MaxOutboundUnknownRawJSONBytes)},
		},
		{
...
func TestTaskEventToProtoWithFailureRejectsOverCapShortPublicCodeStrings(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "unknown raw method",
			payload: domain.UnknownRawEvent{Method: overCap(domain.MaxSourceLabelBytes), PayloadJSON: `{"redacted":true}`},
		},
	}
