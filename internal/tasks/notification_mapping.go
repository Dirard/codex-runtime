...
func (s *Service) publishUnknownNotification(notification appserver.Notification, connection *appserver.Connection, root map[string]any) {
	...
	payloadJSON, truncated := redactedJSON(notification.Params, domain.MaxOutboundUnknownRawJSONBytes, redactor)
	event, subscribers := s.appendEventLocked(task, domain.UnknownRawEvent{
		Method:      publicTextWithRedactor(redactor, notification.Method, domain.MaxSourceLabelBytes, "unknown"),
