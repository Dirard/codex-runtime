...
func TestCommandWorkspaceLabelsUseWorkspaceRelativeSanitizer(t *testing.T) {
	...
	service.handleNotification(appserver.Notification{
		...
		Params: mustRawMessage(t, map[string]any{
			...
			"command":        []any{"echo", "hello"},
			"workspaceLabel": filepath.Join("..", "outside", "secret.txt"),
		}),
	}, connection)
	service.handleNotification(appserver.Notification{
		Method: "item/started",
		Params: mustRawMessage(t, map[string]any{
			...
			"command":        []any{"echo", "hello"},
			"workspaceLabel": `..\outside\secret.txt`,
		}),
	}, connection)
...
func TestStructuredDropsAndUnknownFallbackDoNotExposeRawPayloads(t *testing.T) {
	...
	service.handleNotification(appserver.Notification{
		Method: "fs/changed",
		Params: mustRawMessage(t, map[string]any{"changedPaths": []string{`C:\Users\me\secret.txt`}}),
	}, connection)
	service.handleNotification(appserver.Notification{
	...
	for _, forbidden := range []string{"chatgpt", "plus", "secret.txt"} {
	...
	unknown, ok := events[1].Payload.(domain.UnknownRawEvent)
	if !ok {
		t.Fatalf("event[1] = %T, want UnknownRawEvent", events[1].Payload)
	}
	if strings.Contains(unknown.PayloadJSON, "sk-proj-abcdefghijklmnopqrstuvwxyz") || strings.Contains(unknown.PayloadJSON, "secret.txt") {
		t.Fatalf("unknown payload was not redacted: %s", unknown.PayloadJSON)
	}
}
...
func TestUnknownRawEventUsesConnectionScopedRegistry(t *testing.T) {
	group := testSessionGroup(t, "sg-1", "ws-1")
	service, err := NewService([]Session{{Group: group, Supervisor: &countingSupervisor{}}})
	if err != nil {
		t.Fatal(err)
	...
	registry.Add("credential-secret-value")
	...
	service.handleNotification(appserver.Notification{
		...
		Params: mustRawMessage(t, map[string]any{
			"threadId": "thread-1",
			"turnId":   "turn-1",
			"message":  "echo credential-secret-value",
		}),
	}, connection)
	...
	if strings.Contains(unknown.PayloadJSON, "credential-secret-value") {
		t.Fatalf("unknown payload leaked connection-scoped secret: %s", unknown.PayloadJSON)
	}
	if !strings.Contains(unknown.PayloadJSON, redact.SensitiveValueMarker) {
		t.Fatalf("unknown payload = %s, want sensitive marker", unknown.PayloadJSON)
	}
}
...
func TestUncorrelatedNotificationsRecordRedactedDiagnosticsOnly(t *testing.T) {
	...
	registry := redact.NewRegistry()
	registry.Add("connection-secret-method")
	harness := testappserver.New(t)
	connection := appserver.NewConnection(harness.Stdin(), harness.Stdout(), group, appserver.ConnectionOptions{
		SensitiveRegistry: registry,
	})
	...
	service.handleNotification(appserver.Notification{
		...
		SessionGroupID: group.SessionGroupID,
		Params: mustRawMessage(t, map[string]any{
			"message": "failed with sk-proj-abcdefghijklmnopqrstuvwxyz at C:/Users/me/.codex/config.toml",
		}),
	}, connection)
	service.handleNotification(appserver.Notification{
		Method:         "vendor/connection-secret-method",
		SessionGroupID: group.SessionGroupID,
		Params: mustRawMessage(t, map[string]any{
			"api_key": "sk-proj-abcdefghijklmnopqrstuvwxyz",
	...
	if strings.Contains(diagnostics[1].Method, "connection-secret-method") ||
	...
	for _, forbidden := range []string{"connection-secret-method", "sk-proj-abcdefghijklmnopqrstuvwxyz", "config.toml", "api_key"} {
...
func TestFingerprintMismatchIsSafeAndDoesNotOverwriteEntry(t *testing.T) {
	...
	changed := startCommandWithContext(group, "client-1", "SECRET prompt changed", []domain.ContextBlock{{
		Kind:        domain.ContextBlockKindApplication,
		SourceLabel: "source",
		Content:     "SECRET context changed",
	}})
	_, err := service.StartTask(context.Background(), changed)
	assertGatewayReason(t, err, domain.ReasonIdempotencyFingerprintMismatch)
	if strings.Contains(err.Error(), "SECRET prompt") || strings.Contains(err.Error(), "SECRET context") {
