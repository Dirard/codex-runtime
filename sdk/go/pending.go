...
func (chat *Chat) respondPending(ctx context.Context, pendingRequestID string, applyResponse func(*pb.RespondChatPendingRequest), opts ...RequestOption) (*pb.RespondChatPendingResponse, error) {
	...
	if chat.workflow != nil {
		...
		workflowRequest := &pb.RespondWorkflowChatPendingRequest{
		...
		applyWorkflowPendingResponse(workflowRequest, request)
		reply, err := chat.client.workflow.RespondWorkflowChatPending(rpcCtx, workflowRequest, chat.client.callOptions...)
		if err != nil {
			return nil, normalizeError(err)
		}
		chat.setStatus(reply.GetStatus())
		return workflowPendingToChatPending(reply), nil
	}
...
func (chat *Chat) InterruptRun(ctx context.Context, runID string, opts ...RequestOption) (*pb.InterruptChatRunResponse, error) {
	...
	if chat.workflow != nil {
		...
		response, err := chat.client.workflow.InterruptWorkflowChatRun(rpcCtx, &pb.InterruptWorkflowChatRunRequest{
			Workflow:        chat.workflow.selector(),
			ChatId:          chat.ID,
			RunId:           runID,
		...
		return workflowInterruptToChatInterrupt(response), nil
	}
